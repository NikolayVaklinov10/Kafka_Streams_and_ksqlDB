...
import com.nikolayvaklinov.kafkaStreams.ProcessorAPI.processors.DigitalTwinValueTransformerWithKey;
import com.nikolayvaklinov.kafkaStreams.ProcessorAPI.serialization.json.JsonSerdes;
                ...
                builder
                        ...
                        .flatMapValues(
                                (key, reported) -> {
                                    ...
                                    if (reported.getWindSpeedMph() > 65 && reported.getPower() == TurbineState.Power.ON) {
                                        log.info("high winds detected. sending shutdown signal");
                                        TurbineState desired = TurbineState.clone(reported);
                                        desired.setPower(TurbineState.Power.OFF);
                                        desired.setType(TurbineState.Type.DESIRED);
